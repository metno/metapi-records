...
import models.{ RecordsResponse, Record }

/**
 * Creating a JSON representation of records data
 */
class RecordsJsonFormat extends BasicJsonFormat {

  implicit val recordWrites = Json.writes[Record]

  implicit val recordsResponseWrites: Writes[RecordsResponse] = (
    ...
    (JsPath \ ApiConstants.DATA_NAME).write[Seq[Record]]
  )(unlift(RecordsResponse.unapply))
   ...
   */
  def format[A](start: DateTime, data: List[Record])(implicit request: Request[A]): String = {
    val size = data.size
    val duration = new Duration(DateTime.now.getMillis() - start.getMillis())
    val response = new RecordsResponse(
