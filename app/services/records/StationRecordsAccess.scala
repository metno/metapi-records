...
class StationRecordsAccess extends ProdRecordsAccess {
  ...
  private object recordsExec {
    ...
    def apply(qp: RecordsQueryParameters): List[Record] = {
...
//        Logger.debug(query)
      ...
      recs
        ...
        )).distinct.sortBy(r => (r.elementId, r.county, r.month))
  ...
  override def records(qp: RecordsQueryParameters): List[Record] = recordsExec(qp)
}
